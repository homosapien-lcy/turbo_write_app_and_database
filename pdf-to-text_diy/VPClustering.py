...
for VP_collection_fn in VP_collection_fn_arr:
    ...
    for batch_cur_phrase_collection in chunked(cur_phrase_collection, embedding_processing_batch_size):
        ...
        for i in range(0, len(batch_cur_segment_vec_arr) + clustering_processing_batch_size, clustering_processing_batch_size):
            batch_start = i
            batch_end = i + clustering_processing_batch_size
            batch_segment_vec_arr = batch_cur_segment_vec_arr[batch_start:batch_end, :]

            # use to fit minibatch kmeans
            # check for empty batch
            if len(batch_segment_vec_arr) > 0:
                mini_batch_kmeans.partial_fit(batch_segment_vec_arr)

    end = time.time()
    print("training", VP_collection_fn, "took", end - start, "seconds")

# ----------------------------------------------------------------------

# save the kmeans model
print("finish training, saving the model")
model_file_name = subject + "_kmeans_model.pkl"
savePythonObject(mini_batch_kmeans, output_folder + model_file_name)
