...
fasttext_folder = 'fastText-0.9.2/'
...
def processSegmentBatch(sentence_batch,
                        batch_id_start, batch_id_end,
                        sentence_database_embedding,
                        fast_text_path, model_bin,
                        num_group):
    batch_embedding_LUT = getEmbeddingLUTFromFastText(
        sentence_batch, fast_text_path, model_bin)

    n_group_segment = elementNGroup(sentence_batch, num_group)
    n_group_segment = [' '.join(seg) for seg in n_group_segment]

    id_of_elements_in_batch = range(batch_id_start, batch_id_end + 1)
    n_group_index = elementNGroup(id_of_elements_in_batch, num_group)

    batch_segment_vec_arr = sentence_database_embedding.embedSentenceWithEmbeddingLUTArrMP(
        n_group_segment, batch_embedding_LUT, default_IDF_weight=0)

    return list(n_group_index), batch_segment_vec_arr

# save a batch of segments


def saveSegmentBatch(batch_sentence_text,
                     batch_sentence_embedding_id_arr,
                     batch_sentence_embedding_arr,
                     batch_id_start, batch_id_end,
                     save_folder, output_prefix):
    db_file_index_prefix = str(batch_id_start) + '_' + str(batch_id_end)
    total_prefix = output_prefix + '_' + db_file_index_prefix

    savePythonObject(batch_sentence_text,
                     save_folder + total_prefix + '_sentence_db.pkl')
    savePythonObject(batch_sentence_embedding_id_arr,
                     save_folder + total_prefix + '_sentence_embedding_mat_id.pkl')
    np.save(save_folder + total_prefix + '_sentence_embedding_mat.npy',
            np.array(batch_sentence_embedding_arr))

# combine process and batch


def processAndSaveSegmentBatch(text_batch, segment_batch,
                               batch_id_start, segment_index,
                               sentence_database_embedding,
                               fast_text_path, model_bin, num_group,
                               save_folder, output_prefix):
    # process to id and embeddings
    batch_sentence_embedding_id_arr, batch_sentence_embedding_arr = processSegmentBatch(text_batch,
                                                                                        batch_id_start, segment_index,
                                                                                        sentence_database_embedding,
                                                                                        fast_text_path, model_bin,
                                                                                        num_group)
    ...
    # save part of db
    saveSegmentBatch(segment_batch,
                     batch_sentence_embedding_id_arr,
                     batch_sentence_embedding_arr,
                     batch_id_start, segment_index,
                     save_folder, output_prefix)
