...
# this changes the list_to_pop


def popBatchFromList(list_to_pop, batch_size):
    batch = []
    # edge case for list size < batch size
    loop_num = min(len(list_to_pop), batch_size)
    for i in range(0, loop_num):
        batch.append(list_to_pop.pop(0))
