...
# regex for pubmed
pubmed_id_regex = re.compile(r'<ArticleId IdType="pubmed">(.*?)<\/ArticleId>')
pubmed_article_regex = re.compile(r'<PubmedArticle>(.*?)<\/PubmedArticle>')
pubmed_title_regex = re.compile(r'<ArticleTitle>(.*?)<\/ArticleTitle>')
pubmed_abstract_regex = re.compile(r'<Abstract>(.*?)<\/Abstract>')
pubmed_pubdate_regex = re.compile(r'<PubDate>(.*?)<\/PubDate>')
pubmed_year_regex = re.compile(r'<Year>(.*?)<\/Year>')
pubmed_month_regex = re.compile(r'<Month>(.*?)<\/Month>')
pubmed_day_regex = re.compile(r'<Day>(.*?)<\/Day>')
pubmed_journal_regex = re.compile(r'<Title>(.*?)</Title>')
pubmed_journal_abbrev_regex = re.compile(
    r'<ISOAbbreviation>(.*?)</ISOAbbreviation>')
...
def extractRegex(regex_to_extract, html_content_article_segment):
    content = re.findall(regex_to_extract, html_content_article_segment)
    return remove_tags(content[0])


def guardExtractRegex(regex_to_extract, html_content_article_segment):
    content = re.findall(regex_to_extract, html_content_article_segment)
    if len(content) == 0:
        return "N/A"
...
def extractYMD(html_content_article_segment):
    YMD = guardExtractRegex(pubmed_pubdate_regex,
                            html_content_article_segment)
    ...
    year = guardExtractRegex(pubmed_year_regex, html_content_article_segment)
    month = guardExtractRegex(pubmed_month_regex, html_content_article_segment)
    day = guardExtractRegex(pubmed_day_regex, html_content_article_segment)
...
def divideXmlByArticle(html_content):
    html_content_no_line_break = ''.join(html_content.split('\n'))
    article_info_arr = re.findall(
        pubmed_article_regex, html_content_no_line_break)

    article_info_dict = {}
    for article_info in article_info_arr:
        PMID = guardExtractRegex(pubmed_id_regex, article_info)
